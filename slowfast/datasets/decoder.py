...
def my_get_start_end_idx(video_size, clip_size, clip_idx, num_clips):
    random_idx = random.uniform(0,4)
    cur_idx = int(random_idx)
    if cur_idx == 0:
       start_idx = random.uniform(1,20)
       end_idx = max(video_size/5*4,start_idx)
    elif cur_idx == 1:
       start_idx = random.uniform(video_size/6,video_size/5)
       end_idx = random.uniform(video_size/5*4,video_size/6*5)
    elif cur_idx == 2:
       start_idx = random.uniform(video_size/6,video_size/5)
       end_idx = max(video_size-10,start_idx)
    else:
       start_idx = 1
       end_idx = max(video_size-10,start_idx)
       
    #print("start_end_idx....................",start_idx,end_idx)
    return start_idx, end_idx
...
def pyav_decode(
    ...
    fps = float(container.streams.video[0].average_rate)      # 得到的视频的fps
    frames_length = container.streams.video[0].frames         # 视频的帧数
    duration = container.streams.video[0].duration            # 视频的时长
    #print('fps: {}, frames_length: {}, duration: {}'.format(fps, frames_length, duration))

    if duration is None:
        # If failed to fetch the decoding information, decode the entire video.
        decode_all_video = True
        video_start_pts, video_end_pts = 0, math.inf         # 时长是0时，start=0，wnd=inf
    else:
        # Perform selective decoding.
        decode_all_video = False
        start_idx, end_idx = my_get_start_end_idx(
            frames_length,
